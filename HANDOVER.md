...
- **2026-07-06: レイヤー順序を包括的に見直し、写真がAdmArea（行政区画）の下に隠れる不具合を修正**
  ...
  - 実タイル確認（`stars.optgeo.org/bvmap`を各ズームで直接fetchしデコード）: 樽前山付近のz12/13タイルには一時AdmAreaが出現しなかったが、これはそのタイル固有の現象。z7（北海道全体）・z14以降では少数の大きなポリゴンが陸地をほぼ全面的に不透明白で覆うことを確認済み（後述のズーム7以下の指摘で判明）
...
- **2026-07-06: ズーム7以下で写真が見えない件を調査 → レイヤー順序ではなく不透明度の問題と判明**
  - ユーザー指摘: 「ズームレベル7以下でAdmAreaが写真の上に載ってしまっている」
  - Playwrightで`bvmap-行政区画`の位置関係を再検証: レイヤー順序は正しく維持されていた（`seamlessphoto`は`bvmap-行政区画`より後＝上）。北海道全体で`raster-opacity`を1.0まで上げると鮮やかな緑の航空写真が正しく表示されることを確認
