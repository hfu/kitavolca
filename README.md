...
- 測量法第30条第1項に基づく使用承認済み: **測量法に基づく国土地理院長承認（使用）R 8JHs 207**（令和8年7月14日付、国地情使第207号。使用期間は承認後6か月間）
  ...
  - 承認条件(2)により、Web公開時はサイトURLを国土地理院へ報告すること
    - 公開サイト: `https://hfu.github.io/kitavolca/`（GitHub Pages、`docs/` を公開）
    - 報告文案（未送付。送付方法は承認書に明記が無いため、申請時と同じオンライン申請システム経由か、国土地理院への問い合わせ窓口経由かを事前に確認してから送付すること）:

      ```
      件名: 測量成果使用承認（R 8JHs 207）に基づく公開URLの報告

      国土地理院長 殿

      令和8年7月14日付・国地情使第207号（申請ID: 91qoqcaa7y_20260706_044610）
      にて使用承認をいただきました基本測量成果（火山基本図データ・火山土地条件
      図データ）について、承認条件(2)に基づき、Web上での公開サイトURLを
      下記のとおり報告いたします。

      承認番号: 測量法に基づく国土地理院長承認（使用）R 8JHs 207
      使用目的: システム「PMTiles for VBM and VLCM」作成のため
      公開URL: https://hfu.github.io/kitavolca/
      公開日: （公開開始日を記入）

      成果品には承認条件(1)に基づき、上記承認文言をサイト内の地図属性
      （attribution）として明示しております。
